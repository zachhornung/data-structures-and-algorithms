...
const sortByChildren = (charArray) => {
  charArray.sort(function(l,r){
    if (l.children.length > r.children.length){
      return 1;
    }else if (l.children.length < r.children.length){
      return -1;
    }else{
      if (l.house > r.house){
        return 1;
      }else if (l.house < r.house){
        return -1;
      }else {
        return 0;
      }
...
const houseSize = (arr) => {
  ...
  const houses = [];
  for (let i in arr){
    if (!houses.includes(arr[i].house)){
      houses.push(arr[i].house);
    }
  }
  for (let i in houses){
    sizes.push({house: houses[i], members: 0});
  }
  for (let i in arr){
    for (let j in sizes){
      if (arr[i].house === sizes[j].house){
        sizes[j].members = sizes[j].members + arr[i].children.length;
        if(arr[i].spouse){
          sizes[j].members = sizes[j].members + 2;
        } else {
          sizes[j].members = sizes[j].members + 1;
        }
      }
    }
  }
