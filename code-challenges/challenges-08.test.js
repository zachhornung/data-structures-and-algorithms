...
function returnTen(str){
  ...
  return strArr.splice(strArr.length - 10);
}
...
const containsAnd = (arr) => {
  return arr.filter(index => index.includes('and'));
};
...
const filterStringsWithVowels = (arr) => {
  return arr.filter(index => index.match(/[aeiouAEIOU]/g));
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(index => index.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  const results = [];
  const accepted =  arr.filter(index => index.baseStat > minBaseStat);
  accepted.forEach(index => results.push(index.stat.name));
  return results;
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(index => !index.children);
};
...
const evenOddNumericValues = (arr) => {
  const nums = arr.filter(index => typeof index === 'number');
  return nums.map(index => {
    if(index % 2 === 0){
      return 'even';
    }else {
      return 'odd';
