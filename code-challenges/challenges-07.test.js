...
const charCode = (arr) => {
  let newArr = arr.map(char => char.charCodeAt(0));
  return newArr;
};
...
const extractAbilities = (arr) => {
  let newArr = arr.map(position => {
    return position.ability.name;
  });
  return newArr;
};
...
const extractStats = (arr) => {
  // let newArr = arr.map(position => {name: position.stat.name, total: position.effort + position.baseStat});
  // return newArr;
};
