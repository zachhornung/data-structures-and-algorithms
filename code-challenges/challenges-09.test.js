...
const countNumberOfElements = (arr) => {
  return arr.reduce((element, num) => num++, 0);
};
...
const returnNames = (arr) => {
  return arr.reduce((startWithNewArr, person) => {
    startWithNewArr.push(person.name);
    return startWithNewArr;
  }, []);
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((a, number) => {
    return a + (isPrime(number));
  }, 0);
};
