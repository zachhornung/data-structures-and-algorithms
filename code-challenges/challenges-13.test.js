...
const longestString = (arr) => {
  ...
  const arrOfLengths = arr.map(str => str.length);
  return arrOfLengths.reduce((acc, curr, index) => {
    if (arrOfLengths[index] > arrOfLengths[acc]){
      acc = index;
    }
    return acc;
  }, 0);
...
const unenrollBrook = (arr) => {
  return arr.reduce((acc, curr) => {
    acc.push(curr.filter(item => !item.includes('Brook')));
    return acc;
  }, []);
};
...
const sortByDay = (arr) => {
  // const results = [];
  // results.push(daysOfWeek.reduce((acc, curr, index) => {
  //   if (curr.test(arr)){
  //     const i = arr.findIndex(curr);
  //     acc.push(arr[i]);
  //   }else {
  //     acc.push('');
  //   }
...
const characterByIndex = (arr) => {
  return arr.reduce((acc, curr, index) => {
    acc.push(curr.slice(index, index+1));
    return acc;
  }, []);
};
