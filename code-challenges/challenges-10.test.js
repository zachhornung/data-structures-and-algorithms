...
const count = (target, input) => {
  return input.reduce((acc, curr) => {
    // step 1: do work;
    // step 2: change the accumulator, or leave it alone.
    // step 3: we always have to return the accumulator. this is usually done after it has been altered in whichever way specified.
    acc += curr.reduce((acc2, curr2) => { // notice how acc 1 isbeing added to the result of the second reduce (aka the return of the second reduce, the running total of matches)
      if (curr2 === target){
        acc2 += 1;
      }
      return acc2;
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.reduce((acc, curr) => {
    acc.push(curr.reduce((acc2, curr2) => {
      if(curr2 % 5 === 0 && typeof curr2 ==='number'){
        acc2.push(Math.pow(2, curr2));
      }
      return acc2;
    }, []));
    return acc;
...
let findShortest = (data) => {
  const shortest =  data.reduce((acc, curr) => {
    if(parseInt(curr.height) < acc.height){
      acc = curr;
    }
    return acc;
  });
